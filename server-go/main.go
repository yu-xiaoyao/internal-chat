...
func loadRoomConfig() {
	...
	roomManager = &service.JsonRoomManager{
...
func handleWebSocket(w http.ResponseWriter, r *http.Request) {
	...
	var turns []service.TurnInfo
	// 兼容旧版本
	if roomId == "ws" || roomId == "" {
		roomId = ""
	} else {
		// 验证房间密码
		if roomConfig, exists := roomManager.GetByRoomId(roomId); !exists || pwd == "" || strings.ToLower(roomConfig.Pwd) != strings.ToLower(pwd) {
			roomId = ""
		} else {
	...
	// 注册用户
	currentId := service.RegisterUser(ip, roomId, conn, func(key string) (*http.Cookie, error) {
		return r.Cookie(key)
	})
	...
	for {
		...
		// 验证消息
		if message.Type == "" || message.UID == "" || message.TargetID == "" {
			continue
		}

		// 获取发送者和接收者
		me := service.GetUser(ip, roomId, message.UID)
		target := service.GetUser(ip, roomId, message.TargetID)
		...
		case ReceiveTypeUpdateNickname:
			...
			success := service.UpdateNickname(ip, roomId, message.UID, data.Nickname)
			if success {
				// 通知所有用户昵称更新
				for _, user := range service.GetUserList(ip, roomId) {
...
// 发送用户ID
func socketSendUserId(conn *websocket.Conn, id, roomId string, turns []service.TurnInfo) {
